  $(document).ready(function(){
    ...
    if (localStorage.getItem('userdetails') === null) {  
        ...
        var userDetails = {firstName:"John", lastName:"Smith", dob:"1987-09-14",address1:"Buenos Ayres Drive", address2:"Strandhill", address3:"Co. Sligo"};
    ...
    if ($('#pdetails').length > 0) {
        ...
        $('input[name="address1"]').val(userDetails.address1);
        $('input[name="address2"]').val(userDetails.address2);
        $('input[name="cardname"]').val(userDetails.firstName + " " + userDetails.lastName);
    }
    //** 
    ...
    $('form[name="login"]' ).submit(function ( event ) {
            ...
            if (email=="jsmith@email.com" && password=="password1")
            {
    ...
    // Wait for submit button to be clicked on register form
...
var price = [];
// JAVASCRIPT
if(localStorage.getItem('checkout') == null || localStorage.getItem('checkout') == 0){
    names = [];
    price = [];
}
else{
...
(function(){
    let login = document.getElementById('checkLogin');
    let loggedin = localStorage.getItem('loggedIn');
    if(loggedin == 0){
        login.setAttribute('href', 'login.html')
    }
    else{
        login.setAttribute('href', 'checkout.html')
    }
...
function shop(event){
    ...
    price.push(event.target.attributes[3].nodeValue);
